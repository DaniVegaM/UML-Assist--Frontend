    ...
    return (
        <div>
            ...
            <input
                ...
                className={`w-full px-3 py-2 border dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 ${error ? 'border-red-500' : 'border-gray-300'}`}
            />
            {
                error ? 
                <div className="h-5 mt-1">
                {error && (
                    <p className="text-red-500 text-sm dark:text-red-400 font-bold">{ error }</p>
                )}
            </div>
            : 
            <div></div>
